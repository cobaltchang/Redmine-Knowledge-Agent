...
# Optional dependencies - imported at module level for easier mocking
try:
...
class ImageProcessor(BaseProcessor):
    ...
    def process(self, file_path: Path) -> ExtractedContent:
        ...
        if pytesseract is None or Image is None:
            return self._create_error_result("OCR dependencies not available: pytesseract or PIL not installed")
...
class PdfProcessor(BaseProcessor):
    ...
    def process(self, file_path: Path) -> ExtractedContent:
        ...
        if fitz is None:
            return self._create_error_result("PDF dependencies not available: PyMuPDF not installed")
...
class DocxProcessor(BaseProcessor):
    ...
    def process(self, file_path: Path) -> ExtractedContent:
        ...
        if Document is None:
            return self._create_error_result("DOCX dependencies not available: python-docx not installed")
    ...
    def process(self, file_path: Path) -> ExtractedContent:
        ...
        try:
            if suffix == ".csv":
                ...
                if openpyxl is None:
                    return self._create_error_result("Spreadsheet dependencies not available: openpyxl not installed")
